...
import { ButtonSend, DivInputsIntervalos, DivInteracao1B, TituloInteracao } from './style'
import axios from 'axios'

const checarInterlace = async (intervaloA, intervaloB) =>{
   
 const {data} = await axios.get(`http://localhost:8080/interlace?intervaloA=${intervaloA}&intervaloB=${intervaloB}`);
        ...
        <DivInteracao1B>
            ...
            <DivInputsIntervalos>
                <div>
                    ...
                    <input type="number" name="intervalo11" value={intervalo11} onChange={(e)=>{if(e.target.value.length>5){return null}setIntervalo11(e.target.value)}}/>
                    </div>
                    <div>
                    <label htmlFor="intervalo12">Fim: </label>
                    <input type="number" name="intervalo12"value={intervalo12} onChange={(e)=>{if(e.target.value.length>5){return null}setIntervalo12(e.target.value)}}/>
                    </div>
                    </div>
                </div>
                <div>
                <h5>Selecione o segundo intervalo</h5>
                    ...
                    <input type="number" name="intervalo21" value={intervalo21} onChange={(e)=>{if(e.target.value.length>5){return null}setIntervalo21(e.target.value)}}/>
                    </div>
                    <div>
                    <label htmlFor="intervalo12">Fim: </label>
                    <input type="number" name="intervalo12"value={intervalo22} onChange={(e)=>{if(e.target.value.length>5){return null}setIntervalo22(e.target.value)}}/>
                    </div>
                    </div>

                </div>
            </DivInputsIntervalos>
            <ButtonSend  active onClick={()=>{checar(`${intervalo11},${intervalo12}`, `${intervalo21},${intervalo22}`)}}>Interlaçam?</ButtonSend>
